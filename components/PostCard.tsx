...
export function PostCard({ title, summary, slug, date, tags, cover, featured }: PostCardProps) {
  return (
    <Link href={`/posts/${slug}`} className="group block h-full">
      <article className={`relative overflow-hidden rounded-3xl glass-card transition-all duration-500 hover:-translate-y-2 cursor-pointer h-full flex flex-col ${featured ? 'lg:flex-row' : ''}`}>
        {/* 渐变背景装饰 */}
